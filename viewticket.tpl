    ...
    {/if}
    {if !$invalidTicketId}
        <div class="panel panel-default panel-ticket-reply  hidden-print">
            <div class="panel-heading" id="ticketReply1"   >
                <div class="collapse-icon pull-right">
                    <i class="fa fa-caret-{if !$postingReply}down{else}up{/if}"></i>
                </div>
                <h4 class="panel-title">
                    <i class="fa fa-pen"></i> &nbsp; {$LANG.supportticketsreply}
                </h4>
            </div>
            <div class="{if !$postingReply} collapse{/if}" id="ticketReplyBox1"  aria-labelledby="ticketReply1">
                        ...
                        <div class="row form-group">
                            ...
                            <div class="col-sm-9">
                                <div class="file-input form-control">
                                    ...
                                    <span class="file-input-text text-light">{$LANG.support_no_file_selected}</span>
                                </div>
                                <div id="fileUploadsContainer"></div>
                            </div>
                            <div class="col-sm-3">
                                <button type="button" class="btn btn-primary-faded btn-block add-extra-attachement" data-nofiletext="{$LANG.support_no_file_selected}" data-selectfiletext="{$LANG.select_file}" data-removetext="{$LANG.orderForm.remove}">
                    ...
                    <div class="ticket-reply-attachments">
                        ...
                        <ul>
                            {foreach from=$reply.attachments key=num item=attachment}
                                ...
                                {else}
                                    <li><a href="dl.php?type={if $reply.id}ar&id={$reply.id}{else}a&id={$id}{/if}&i={$num}"><i class="fa fa-download"></i> {$attachment}</a></li>
                                {/if}
                            {/foreach}
                        </ul>
                    </div>
...
<script>
    jQuery(document).ready(function(){
            jQuery("#ticketReply1").on('click',function(){
                jQuery("#ticketReplyBox1").slideToggle();
            });
    });
